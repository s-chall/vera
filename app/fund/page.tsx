...
import { payouts, pool } from "@/lib/content";
    ...
    <main id="main-content" className="page-shell fund-page">
      ...
      <div className="fund-layout">
        <section className="pool-card" aria-labelledby="pool-title">
          ...
          <strong>{pool.balance} <small>BTC</small></strong>
          <p>Protected by a public contract. The next automated payout closes in {pool.closesInDays} days.</p>
          <div className="pool-progress"><span style={{ width: `${pool.percent}%` }}/></div>
          <div className="pool-label"><span>{pool.balance} BTC contributed</span><span>{pool.goal} BTC goal</span></div>
          <FundDialog />
      ...
      <section className="ledger-section" id="ledger" aria-labelledby="ledger-title">
        <header><div><span className="overline">On-chain history</span><h2 id="ledger-title">Public payout ledger</h2><p>Amounts and aliases are visible. Legal identities stay private. These rows are placeholder data; no epoch has run.</p></div><button className="secondary-button">View contract <ExternalLink aria-hidden="true"/></button></header>
