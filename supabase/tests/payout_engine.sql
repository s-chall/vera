...
begin
  ...
  values
    (user_one, '00000000-0000-0000-0000-000000000000', 'authenticated', 'one@example.test', 'x'),
    (user_two, '00000000-0000-0000-0000-000000000000', 'authenticated', 'two@example.test', 'x');
  ...
  values (now() - interval '15 days', now() - interval '1 day') returning id into epoch_id;
  ...
  values
    (article_one, repeat('a', 64), 'view', true, now() - interval '2 days'),
    (article_one, repeat('b', 64), 'view', true, now() - interval '2 days'),
    (article_one, repeat('a', 64), 'like', true, now() - interval '2 days'),
    (article_two, repeat('c', 64), 'view', true, now() - interval '2 days'),
    (article_two, repeat('d', 64), 'view', true, now() - interval '2 days');

  perform public.finalize_payout_epoch(epoch_id, 600000000);

  select sum(payout_atomic) into total_paid from public.epoch_allocations where epoch_allocations.epoch_id = epoch_id;
  select payout_atomic into first_paid from public.epoch_allocations
    where epoch_allocations.epoch_id = epoch_id and journalist_id = journalist_one;
  select payout_atomic into second_paid from public.epoch_allocations
    where epoch_allocations.epoch_id = epoch_id and journalist_id = journalist_two;
  ...
  perform public.finalize_payout_epoch(epoch_id, 600000000);
  if (select count(*) from public.epoch_allocations where epoch_allocations.epoch_id = epoch_id) <> 2 then
